...
export class ServiceTradingProxy {
    ...
    getAllCryptoBalance(): Observable<PRGetAllMemberCryptoBalanceForMenuOutput> {
        let url_ = this.baseUrl + '/api/services/app/MntMemberTrading/GetAllCryptoBalance';
        url_ = url_.replace(/[?&]$/, '');
        ...
        return this.http.request('get', url_, options_).pipe(_observableMergeMap((response_: any) => this.processGetAllCryptoBalance(response_))).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetAllCryptoBalance(response_ as any);
                } catch (e) {
                    return _observableThrow(e) as any as Observable<PRGetAllMemberCryptoBalanceForMenuOutput>;
                }
            } else {
                return _observableThrow(response_) as any as Observable<PRGetAllMemberCryptoBalanceForMenuOutput>;
            }
        }));
    }

    protected processGetAllCryptoBalance(response: HttpResponseBase): Observable<PRGetAllMemberCryptoBalanceForMenuOutput> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
                (response as any).error instanceof Blob ? (response as any).error : undefined;
        ...
        if (status === 200) {
            return Helpers.blobToText(responseBlob).pipe(_observableMergeMap((_responseText: string) => {
                let result200: any = null;
                let resultData200 = _responseText === '' ? null : JSON.parse(_responseText, this.jsonParseReviver);
                result200 = PRGetAllMemberCryptoBalanceForMenuOutput.fromJS(resultData200);
                return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return Helpers.blobToText(responseBlob).pipe(_observableMergeMap((_responseText: string) => Helpers.throwException('An unexpected server error occurred.', status, _responseText, _headers)));
