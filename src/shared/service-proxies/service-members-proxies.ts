...
import { GetMemberDetailRequestDto } from './dto/members/mntMemberFiat/GetMemberDetailRequestDto';
...
export class ServiceMembersProxy {
    ...
    */
    ...
    getMemberDetailRequest(requestId: number | undefined): Observable<GetMemberDetailRequestDto> {
        let url_ = this.baseUrl + '/api/services/app/MntMemberFiat/GetMemberDetailRequest?';
        if (requestId === null) {
    ...
    getGetMemberFullName(): Observable<any> {
        let url_ = this.baseUrl + '/api/services/app/MntMemberFiat/GetMemberFullName';
        url_ = url_.replace(/[?&]$/, '');
        ...
        return this.http.request('get', url_, options_).pipe(_observableMergeMap((response_: any) => this.processGetMemberFullName(response_))).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetAllBanksForSelect(response_ as any);
                } catch (e) {
    ...
    protected processGetMemberFullName(response: HttpResponseBase): Observable<any> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
                (response as any).error instanceof Blob ? (response as any).error : undefined;
    ...
    downloadDepositFile(userId: number | undefined, fileId: number | undefined): void {
        ...
        url_ += 'userId=' + encodeURIComponent('' + ((userId === null || userId === undefined) ? '' : userId)) + '&';
        url_ += 'fileId=' + encodeURIComponent('' + ((fileId === null || fileId === undefined) ? '' : fileId)) + '&';
        url_ = url_.replace(/[?&]$/, '');
        location.href = url_;
    }
}
